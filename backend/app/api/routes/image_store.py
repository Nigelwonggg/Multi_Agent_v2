...
def _get_domain_store(domain: Optional[str]):
    try:
        return image_factory.get_store_by_name(domain or "data_science")
...
async def get_filter_options(
    ...
    image_store = _get_domain_store(domain)
...
async def delete_image_document(
    ...
    if not success:
        logger.error(f"Failed to delete image document {doc_id} from domain {domain}")
        raise HTTPException(status_code=404, detail="Image document not found")

    logger.info(f"Image document {doc_id} deleted successfully from domain {domain}")
...
async def get_image_documents_by_ids(
    ...
    """Get multiple image documents by their doc_ids."""
    try:
        image_store = _get_domain_store(domain)
        logger.info(f"Fetching {len(request.doc_ids)} image documents by IDs in domain {domain}")
...
async def get_image_documents_by_domain(
    ...
    try:
        logger.info(f"Fetching image documents by domain: {request.docs_by_domain}")
