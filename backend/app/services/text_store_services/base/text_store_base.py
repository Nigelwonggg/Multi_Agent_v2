"""
Base Text Store Service

Abstract base class for all text store implementations.
...
class BaseTextStoreService(ABC):
    """Abstract base class for text store services"""
    
    def __init__(self, domain: str, collection_suffix: str = ""):
        self.domain = domain
        self.logger = get_logger(f"services.text_store.{domain}")
        ...
        # Move directory creation after logger info to be slightly more robust
        ...
        self.logger.info(f"🚀 Initializing Chroma for domain '{domain}' at {persist_dir}")
        ...
        try:
            ...
            self.logger.info(f"✅ {domain} text store initialized: {persist_dir}")
        ...
        self.text_retriever = self.text_store.as_retriever(
            search_type="mmr", 
    ...
    def cleanup(self) -> None:
        ...
        try:
            if hasattr(self, 'text_store') and self.text_store:
                self.logger.info(f"🧹 Performing aggressive cleanup for domain '{self.domain}'")
                ...
                # Try to find client inside the text_store object too
                client = getattr(self.text_store, '_client', None)
                if client:
                    try:
                ...
                # This is a bit hacky but helps break cycles and release handles on Windows
                internal_attrs = ['_client', '_collection', '_api']
                for attr in internal_attrs:
                    if hasattr(self.text_store, attr):
                        try:
                            setattr(self.text_store, attr, None)
                        except:
                            pass
            ...
            self.logger.error(f"❌ Error during aggressive cleanup for '{self.domain}': {str(e)}")
    ...
    def get_all_documents(self) -> List[Dict[str, Any]]:
        ...
        try:
            data = self.text_store._collection.get()
            docs = []
            
            for i, doc_id in enumerate(data["ids"]):
                summary_text = data["documents"][i]
                metadata = data["metadatas"][i] or {}
                
                # Convert page number if needed
                ...
                doc_data = self._format_document_output(i + 1, doc_id, summary_text, metadata)
    ...
    def _format_document_output(self, id: int, doc_id: str, summary_text: str, metadata: Dict) -> Dict[str, Any]:
    ...
    def get_documents_paginated(
        ...
        if search:
            filtered_docs = [
                ...
                if (search.lower() in doc["summary_text"].lower() or 
                    search.lower() in doc["raw_text"].lower() or 
    ...
    def add_document(self, doc_data: Dict[str, Any]) -> str:
        ...
        doc_id = doc_data.get("doc_id") or f"{self.domain}_doc_{uuid.uuid4().hex[:8]}"
        
        # Prepare metadata with domain-specific schema
        metadata = self._prepare_metadata(doc_id, doc_data)
        
        new_doc = Document(
            page_content=doc_data["summary_text"],
            metadata=metadata
        )
        
        self.text_store.add_documents([new_doc], ids=[doc_id])
    ...
    def update_document(self, doc_id: str, doc_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.text_store.update_document(document_id=doc_id, document=updated_doc)
            self.logger.info(f"Updated document: {doc_id}")
            return True
        except Exception as e:
            self.logger.error(f"Error updating document {doc_id}: {str(e)}")
            self.logger.debug(f"Document data: {doc_data}")
            return False
    ...
    def delete_document(self, doc_id: str) -> bool:
        ...
        try:
            ...
            self.text_store.delete(ids=[doc_id])
    ...
    def search_documents_by_content(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            results = self.text_store.similarity_search(query, k=limit)
    ...
    def search_documents_by_query(self, query: str, limit: int = 10) -> List[Document]:
        """Search documents using retriever (for RAG)"""
        try:
            results = self.text_retriever.invoke(query)
            
            if limit and len(results) > limit:
                results = results[:limit]
            
            self.logger.info(f"Retriever search returned {len(results)} results")
            return results
        except Exception as e:
            self.logger.error(f"Error in retriever search: {str(e)}")
            return []
    
    def get_retriever(self):
        """Get the text retriever instance"""
    ...
    def update_retriever_config(self, search_type: str = "similarity_score_threshold", 
        ...
        self.logger.info(f"Updated retriever: {search_type}, kwargs: {search_kwargs}")
    
    def get_documents_by_ids(self, doc_ids: List[str]) -> List[Dict[str, Any]]:
        """Get multiple documents by their doc_ids"""
        try:
            if not doc_ids:
                return []
            ...
            self.logger.info(f"Found {len(matching_docs)} documents out of {len(doc_ids)} requested")
            return matching_docs
        except Exception as e:
            self.logger.error(f"Error getting documents by IDs: {str(e)}")
            return []